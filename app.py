...
def create_groq_client():
    """Create the official Groq client using Streamlit Secrets."""
    try:
...
def analyze_resume(resume_text, job_description):
    """Run the resume review using the official Groq SDK.

    CrewAI remains installed for the project architecture, but the
    actual GPT-OSS request uses Groq's official client directly.
    This avoids the LiteLLM cache_breakpoint issue.
    """
...
"""
    ...
    system_prompt = (
        "You are an evidence-based resume review specialist. "
        "Analyze only the evidence explicitly present in the resume. "
        "Never invent qualifications. Return only valid JSON."
    )
...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div class="sidebar-item">
            <div class="sidebar-label">Agent Framework</div>
